...
主流程是單頁完成：上傳參考圖後，AI 會分析角色外觀、色彩、線條、材質、構圖與整體氣氛，整理成可直接生圖的原創 Prompt；接著生成 8／16／24／32／40 張、依每 16 張組成一張 4×4 總圖、用內建 Cutter 切回各張貼圖，最後打包成 LINE 上架 ZIP。Prompt 仍可手動修改。

「補充貼圖主題／風格／文字語氣」也會影響台詞內容。AI 會依所選張數產生「台詞｜動作」，你可以逐句修改；角色圖片由影像模型生成，確認過的繁體中文台詞再由瀏覽器後製到圖片上。
...
| 項目 | 規格 | 程式是否自動保證 |
|---|---|---|
| 張數 | 8／16／24／32／40 張；每 16 張一張 4×4 總圖 | 是，不完整不給打包 |
